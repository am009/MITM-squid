...
if ($debug == 1) { open (DEBUG, '>>./asciiImages_debug.log'); }
autoflush DEBUG 1;
...
while (<>) {
   ...
   if (($uri =~ /(.*\.(gif|png|bmp|tiff|ico|jpg|jpeg))$/i) && (!($uri =~ /wjk-surface/))&&(!($uri =~ /localhost/))&&(!($uri =~ /127.0.0.1/))&&(!($uri =~ /(.*\.(wjk))/)) ){                      # Image format(s)
      ...
      print $channel."\n";                                                              # Just let it go
      if ($debug == 1) { print DEBUG "Pass: $uri\n"; }                             # Let the user know
   }

   if ($asciify == 1) {                                                          # Do we need to do something?
      if (!($uri =~ /(.*\.(jpg|jpeg))$/i)) {                                          # Select everything other image type to jpg
      ...
      print $channel."$baseURL/$filename-ascii.png\n";
